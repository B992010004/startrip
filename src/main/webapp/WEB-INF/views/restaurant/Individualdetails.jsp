...
<body>
	...
	<script>
		...
		stomp.connect('guest', 'guest', function(frame) {
			...
			//訂閱廣播
			//一開始大家只有廣播
			stomp.subscribe("/brocast/message/" + '${LoginOK.memberid }',
					handleInitP2P);
		});

		function initChat(senderPk, receiverPk) {
			if (!issubscribed(receiverPk)) {
				stomp.subscribe("/target/message/" + senderPk + "/"
						+ receiverPk, handleText);
			...
			if ($('#messages').val()) {
				...
				$('#messages').val("");
				var myText = $(
						"<span class='myText' style='color:#FFFFFF;text-align:center;'></span>")
						.text(message);
				var myTextdiv = $(
						"<div class='myTextDiv' style='border-radius:10px;background-color:#0066FF;float:right;clear:both;max-width:146px;word-wrap: break-word;margin-bottom:5px;'></div>")
						.append(myText);
				...
				//一開始先推到對方廣播
				stomp.send("/app/brocast/message/" + senderPk + "/"
						+ receiverPk, {}, payload);
			}
		}
		...
		function sendMessage(senderPk, receiverPk) {
			...
			if ($('#messages').val()) {
				...
				//stomp.send("/app/chatRoom/" + senderPk + "/" + receiverPk, {}, payload);
				//點對點通道
				stomp.send("/app/chatRoom/" + senderPk + "/" + receiverPk, {},
						payload);
			}
		}
		...
		function handleInitP2P(message) {
			...
			var callBackText = $(
					"<span class='callBackText' style='text-align:center;'></span>")
					.text(JSON.parse(message.body).message);
			var callBackTextdiv = $(
					"<div class='callBackTextDiv' style='border-radius:10px;background-color:#DDDDDD;float:left;clear:both;max-width:146px;word-wrap:break-word;margin-bottom:5px;'></div>")
					.html(callBackText);
			...
			$('#sendText').attr(
					'onclick',
					"sendMessage(" + object.receiverAccount + ","
			...
			if (object.messageStatus == 1) {
				...
				stomp.send("/app/brocast/message/" + object.receiverAccount
						+ "/" + object.senderAccount, {}, payload);
					...
					.ajax({
						...
						success : function(responce) {
							...
							for (i = 0; i < responce.length; i++) {
								...
								var innerRow2content = $('<div><h6>'
										+ responce[i].memberBean.username
										+ '</h6></div>');
								...
								var title = $('<div class="probootstrap_font-18"><h5>'
										+ responce[i].title + '</h5></div>');
								var content = $('<div>' + responce[i].content
										+ '</div>');
								...
								if (responce[i].photoPathList != null) {
									$
											.each(
													responce[i].photoPathList,
													function(idx, photoPath) {
